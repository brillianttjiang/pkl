  ...
  <div class=\"container-fluid\">
	...
	<div class="col-lg-5 col-lg-offset-4 blkg">
		...
		else
		{
			...
			$querykirim = "SELECT * FROM kirim WHERE idkirim='" . $_POST["idkirim"] . "'AND status!=2";
			$resultkirim = mysqli_query($conn, $querykirim);
			$rowkirim=mysqli_fetch_assoc($resultkirim);
			$status=$rowkirim["status"];
			$total=0;
			...
			if ($resultorder) 
			{
				...
				echo "<br>
				ALAMAT: RAYA LANGSEP 56
				<br>
				KOTA  : MALANG
				<br>
				NPWP  : 01.949.374.1.623.000
				...
				<thead>
					...
					$y=1;
					$queryitem = "SELECT * FROM item natural join `order` WHERE idkirim='".$idkirim."'";
					...
					echo"</table>";
					echo"<form method=\"post\" action=\"pklkonfirmasiinvoiceadmin.php\">";
					if($status==0)
					{
						echo"<div class=\"checkbox\">
						<label><input type=\"checkbox\" name=\"konfirmasi\" value=\"1\">Sudah Bayar</label>
					</div><input type=\"hidden\" value=".$idkirim." name=\"idkirim\">";	
					echo"<button class=\"btn btn-primary\" type=\"submit\">Konfirmasi</button></form>";
					}
				if($status==1)
				{
					echo"<div class=\"checkbox\">
					<label><input type=\"checkbox\" value=\"1\" name=\"konfirmasi\" checked disabled>Sudah Bayar</label>
				</div></form>";
			}
			...
			<a href=\"pklcekinvoiceadmin.php\"><button class=\"btn btn-default\" >Back</button></a>";
		} 
	}
...
<!-- $queryorder = "SELECT * FROM order WHERE idkirim='" . $idkirim[$x] . "'";
				$resultorder = mysqli_query($conn, $queryorder);
				$jumlahorder = mysqli_num_rows($resultorder);
